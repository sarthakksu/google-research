...
#     additionally include transitions for the [CLS] and end states, which are used
...
#     labels_with_boundaries = list(labels.items()) + [(start_tag, "[CLS]"), (end_tag, "[SEP]")]
#
...
def allowed_transitions(constraint_type: str, labels: Dict[int, str], tag_set: Dict[int, str] = None) -> (List[Tuple[int, int]], pd.DataFrame):
    ...
    start_tag = 1 #num_labels
    end_tag = 2 #num_labels + 1
    labels_with_boundaries = list(labels.items()) #+ [(start_tag, "[CLS]"), (end_tag, "[SEP]")]

    tag_set_with_boundaries = list(tag_set.items()) #+ [(start_tag, "[CLS]"), (end_tag, "[SEP]")]
...
def is_transition_allowed(constraint_type: str,
    ...
    if to_tag == "[CLS]" or (from_tag == "[SEP]" and to_tag != "[PAD]") or (to_tag == "[PAD]" and not(from_tag == "[SEP]" or from_tag =="[PAD]")) or ((from_tag =="[PAD]" and to_tag !="[PAD]")):
        # Cannot transition into [CLS] or from END
        return False
    if (to_tag == "[PAD]" and (from_tag == "[SEP]" or from_tag =="[PAD]")) or ((from_tag =="[PAD]" and to_tag =="[PAD]")):
        return True
